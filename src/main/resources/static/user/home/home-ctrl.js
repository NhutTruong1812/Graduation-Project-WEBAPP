...
app.controller('home-ctrl', function($scope, $rootScope, $http, $timeout) {
	...
	$scope.listPost = [];
	//load all post (post.card.isProduct: false, post.card.status: 3, post.card.hidden: false, hideCard của user đang login không có bài đăng đó)
	$scope.loadAllPost = async function() {
		let sessionUser = JSON.parse(sessionStorage.getItem("usersession"));
		let resp = await $scope.postAPI.getByProperties({ 'loadAllPost': 1 });
		$scope.listPost = resp;
	...
	$scope.reportCard = function() {
		...
		if (getSelectedValue != null) {
			...
			$scope.reportAPI.create(obj_reportCard).then(resp => {
